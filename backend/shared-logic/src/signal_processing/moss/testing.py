...
async def test_chained_pipeline():  
    ...
    pipeline_config = {  
        "nodes": [  
            {"type": "bandpass filter", "config": {"sfreq": 250.0, "l_freq": 1.0, "h_freq": 40.0}},  
    ...
    print(f"✅ Test 1 passed - Output keys: {list(output.keys())}\n")  
...
async def test_nan_inf_values():  
    ...
    try:  
        output = await run_pipeline(pipeline_config, raw_eeg)  
        print(f"✅ Test 3 passed - Handled NaN/Inf gracefully\n")  
...
async def test_short_data():  
    ...
    try:  
        output = await run_pipeline(pipeline_config, raw_eeg)  
        print(f"✅ Test 4 passed - Short data handled\n")  
    except Exception as e:  
...
async def test_quality_check_only():  
    ...
    output = await run_pipeline(pipeline_config, raw_eeg)  
    print(f"✅ Test 5 passed - Quality check standalone works\n")  
...
async def test_wrong_shape():  
    ...
    try:  
        output = await run_pipeline(pipeline_config, raw_eeg)  
        print(f"⚠️  Test 6 - Should have failed with wrong shape\n")  
    except Exception as e:  
